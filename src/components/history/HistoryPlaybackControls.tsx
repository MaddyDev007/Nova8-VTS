...
type HistoryPlaybackControlsProps = {
...
export function HistoryPlaybackControls({
  ...
  const currentPoint = timeline[currentPointIndex] ?? null

  return (
    <section className='rounded-2xl border border-white/30 bg-white/55 p-4 shadow-lg shadow-slate-900/5 backdrop-blur-xl dark:border-slate-700/70 dark:bg-[#1e293b]/70 dark:shadow-black/20'>
      <h3 className='mb-3 text-lg font-semibold text-slate-900 dark:text-slate-100'>History Playback</h3>

      <div className='flex flex-wrap items-center gap-2'>
        <button
          type='button'
          onClick={onPlay}
          disabled={timeline.length === 0 || isPlaying}
      ...
      <div className='mt-3 space-y-1 text-sm text-slate-700 dark:text-slate-200'>
        <p>
          Point: {timeline.length ? currentPointIndex + 1 : 0} / {timeline.length}
        </p>
        <p>
          Current Timestamp:{' '}
          {currentPoint ? new Date(currentPoint.timestamp).toLocaleString() : 'No journey data'}
        </p>
      </div>
      ...
      <input
        ...
        step={1}
        value={Math.min(currentPointIndex, Math.max(0, timeline.length - 1))}
