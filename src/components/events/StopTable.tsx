...
export function StopTable({ events }: StopTableProps) {
    ...
    <section className='rounded-2xl border border-white/30 bg-white/55 p-4 shadow-lg shadow-slate-900/5 backdrop-blur-xl dark:border-slate-700/70 dark:bg-[#1e293b]/70 dark:shadow-black/20'>
      <div className='mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between'>
        <div>
          ...
          <p className='text-sm text-slate-600 dark:text-slate-300'>Stop incidents and timeline</p>
        </div>
            ...
            <tr className='border-b border-slate-200 text-left text-xs uppercase tracking-[0.1em] text-slate-500 dark:border-slate-700 dark:text-slate-400'>
              ...
              <th className='px-3 py-2 font-semibold'>
                <button type='button' onClick={() => handleSort('duration')}>
                  Stop Duration{sortIndicator('duration')}
                </button>
              </th>
              <th className='px-3 py-2 font-semibold'>
          ...
          <tbody>
            {paginatedEvents.length ? (
              paginatedEvents.map((event) => (
                <tr
                  key={event.id}
                  onClick={() => navigate(`/stop/${event.id}`)}
