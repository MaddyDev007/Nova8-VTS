...
export type IdlingFilterPayload = {
  vehicleId?: string
  minDuration?: number
  startDate?: string
  endDate?: string
  dateRange?: IdlingDateRange
}

type IdlingFiltersProps = {
  vehicles: Array<{ id: string; label: string }>
  onChange: (filters: IdlingFilterPayload) => void
  initialFilters?: IdlingFilterPayload
}
...
export function IdlingFilters({ vehicles, onChange, initialFilters }: IdlingFiltersProps) {
  ...
  return (
    <section className='rounded-2xl border border-white/30 bg-white/55 p-4 shadow-lg shadow-slate-900/5 backdrop-blur-xl dark:border-slate-700/70 dark:bg-[#1e293b]/70 dark:shadow-black/20'>
      <div className='grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5'>
        ...
        <select
          value={dateRange}
          onChange={(event) => setDateRange(event.target.value as IdlingDateRange)}
          className='rounded-xl border border-slate-200 bg-white/80 px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-blue-500 dark:border-slate-600 dark:bg-slate-900/50 dark:text-slate-100 dark:focus:border-[#38bdf8]'
        >
